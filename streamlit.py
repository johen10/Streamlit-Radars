...
xlsx_directory = 'https://github.com/johen10/Streamlit-Radars.git'  # Make sure this directory contains your CSV files

# Get a list of CSV files in the directory
xlsx_files = [f for f in os.listdir('https://github.com/johen10/Streamlit-Radars.git') if f.endswith('.xlsx')]
...
data_path = os.path.join('https://github.com/johen10/Streamlit-Radars.git', selected_file)
        ...
        if len(percentile_player_data) == len(selected_metrics):
            ...
            for metric in selected_metrics:
                if metric in attacking_metrics:
                    slice_colors.append("#DB7093")  # Tomato red for attacking metrics
                elif metric in possession_metrics:
                    slice_colors.append("#4BA543")  # Dodger blue for possession metrics
                elif metric in defending_metrics:
                    slice_colors.append("#2F95F7")  # Lime green for defending metrics
                elif metric in goalkeeper_metrics:
                    slice_colors.append("#2F95F7")  # Gold for goalkeeper metrics
...
# add subtitle
fig.text(
    0.50, 0.93, f"{selected_player} | {selected_position} | {player_team} | Minutes Played: {player_minutes}", size=9,
    ha="center", weight = 'ultralight',  color="#000000"
...
fig.patches.extend([
    ...
    mpl.Rectangle(
        (0.462, 0.05), 0.015, 0.015, fill=True, color="#4BA543",
        transform=fig.transFigure, figure=fig
    ),
    mpl.Rectangle(
        (0.55, 0.05), 0.015, 0.015, fill=True, color="#2F95F7",
        transform=fig.transFigure, figure=fig
    ),
])
